...
#######################################################################################
#
# Usage: boot_patch.sh <INIT_BOOTIMAGE>
#
...
INIT_BOOTIMAGE="$1"
[ -e "$INIT_BOOTIMAGE" ] || abort "$INIT_BOOTIMAGE does not exist!"

# Dump image for MTD/NAND character device boot partitions
if [ -c "$INIT_BOOTIMAGE" ]; then
  nanddump -f init_boot.img "$INIT_BOOTIMAGE"
  BOOTNAND="$INIT_BOOTIMAGE"
  INIT_BOOTIMAGE=init_boot.img
fi
...
ui_print "- Unpacking init_boot image"
./magiskboot unpack "$INIT_BOOTIMAGE"
...
case $? in
  ...
  1 )
    ...
    ;;
  2 )
    ui_print "- ChromeOS init_boot image detected"
    CHROMEOS=true
    ;;
  * )
    abort "! Unable to unpack init_boot image"
    ;;
esac
...
case $((STATUS & 3)) in
  0 )  # Stock init_boot
    ui_print "- Stock init_boot image detected"
    SHA1=$(./magiskboot sha1 "$INIT_BOOTIMAGE" 2>/dev/null)
    cat $INIT_BOOTIMAGE > stock_init_boot.img
    cp -af ramdisk.cpio ramdisk.cpio.orig 2>/dev/null
    ;;
  1 )  # Magisk patched
    ui_print "- Magisk patched init_boot image detected"
    ./magiskboot cpio ramdisk.cpio \
    "extract .backup/.magisk config.orig" \
    "restore"
    cp -af ramdisk.cpio ramdisk.cpio.orig
    rm -f stock_init_boot.img
    ;;
  2 )  # Unsupported
    ui_print "! init_boot image patched by unsupported programs"
    abort "! Please restore back to stock init_boot image"
    ;;
esac
...
for dt in dtb kernel_dtb extra; do
  if [ -f $dt ]; then
    if ! ./magiskboot dtb $dt test; then
      ui_print "! init_Boot image $dt was patched by old (unsupported) Magisk"
      abort "! Please try again with *unpatched* init_boot image"
    fi
    if ./magiskboot dtb $dt patch; then
      ui_print "- Patch fstab in init_boot image $dt"
    fi
  fi
done
...
ui_print "- Repacking init_boot image"
./magiskboot repack "$INIT_BOOTIMAGE" || abort "! Unable to repack boot image"

# Sign chromeos boot
$CHROMEOS && sign_chromeos

# Restore the original boot partition path
[ -e "$BOOTNAND" ] && INIT_BOOTIMAGE="$BOOTNAND"
